...
14. **A rejection column that is present-but-blank is not the same as genuinely zero rejection.** Also: **removing a suppression flag can switch on a fabricated figure downstream.** Closing R-25 was correct about hours, but `APP_DEFAULT_IDEAL_HOURS["TANK"]` then supplied a 500-hour denominator that no one had authorised — producing 46.4% utilisation for VN July and a green 0.0% for KH/WB. **When a metric is unblocked, check that its denominator also resolves to a real, business-supplied value before shipping.** Fix: remove the plant from `APP_DEFAULT_IDEAL_HOURS`; `resolve()` then returns `(None, SRC_NOT_SET)` and utilisation stays suppressed. Having run hours is not the same as having a planned-hours standard to measure them against. *(2026-08-16)* `rejection_tracked = (rej_c >= 0)` was the old guard — it rendered a green `0.00%` whenever a rejection header was found, even if every data cell was empty. Fixed: `rejection_tracked = (rej_c >= 0 and any_rej_nonzero)`. A column that exists but carries no non-zero values produces `rejection_tracked=False` → n/a, not green zero. DR per-date join then supplies the fallback value where available (HDPE May M/C-1: 120 kg from Daily Report May,1 triplet). *(confirmed 2026-08-15)*
