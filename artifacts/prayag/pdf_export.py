...
def _make_footer(generated_str: str, analysis_model: str | None):
    """Build an onPage callback that stamps every page with the generation
    date (dd-mm-yyyy HH:MM), the page number, and — when AI prose was used —
    the analysis-model provenance. Drawn directly on the canvas so it repeats
    on every page of a multi-page report."""
    def _draw(canvas, doc):
        ...
        canvas.drawString(15 * mm, y, f"Generated: {generated_str}")
        # Centre: page number.
        canvas.drawCentredString(w / 2, y, f"Page {canvas.getPageNumber()}")
        # Right: analysis-model provenance (only when AI prose is present).
        right = f"Analysis: {analysis_model}" if analysis_model else "No AI used"
...
def generate_report_pdf(
    ...
    story.append(Spacer(1, 8 * mm))
    prov = f" AI prose by {analysis_model}." if analysis_model else ""
    story.append(Paragraph(
        f"Generated {generated_str} by Prayag Production Analytics — numbers computed "
        f"by deterministic engine, never through AI.{prov}",
